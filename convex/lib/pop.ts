...
export async function evaluatePop(opts: {
  ...
  // host + sessionToken are reconstructed from server-side truth (the declared
  // header for host, the SESSION's stored pst for sessionToken), so the signature
  // verifies only if the client signed that exact host AND the token of the
  // session its cookie maps to — authenticating the host for the caller's
