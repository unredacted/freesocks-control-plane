...
/**
 * The backend's major version. The harness pins a 3.x image; the nightly job
 * overrides it with REMNAWAVE_TEST_IMAGE (a 2.x tag) to prove the 2.x paths.
 */
const BACKEND_MAJOR = Number(/:(\d+)\./.exec(process.env.REMNAWAVE_TEST_IMAGE ?? '')?.[1] ?? 3);
...
describe.skipIf(!BASE_URL || !API_TOKEN)('remnawave management contract (integration)', () => {
  ...
  test('nodes: create a backend row, rename, disable, enable, restart, delete', async () => {
    ...
    expect(restarted.status).toBe(BACKEND_MAJOR >= 3 ? 202 : 200);
