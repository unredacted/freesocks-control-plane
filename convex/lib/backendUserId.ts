 ...
 * uuid and addresses users by a per-backend numeric `id` — and Outline's key ids
 * are per-server small integers too. Two backends both mint user `2`.
 *
 ...
 *
 * The provider itself infers the backend contract from the RAW id's shape (UUID →
 * 2.x body/param names, integer → 3.x), so a mixed fleet keeps working while
 * backends are upgraded one at a time (see convex/lib/backends/remnawave.ts).
 */
 ...
 * backend with the bare integer would "find" an unrelated user, so the fleet
