...
async function insertLiveEdge(
  ...
  await t.run(async (ctx) => {
    ...
    const relayId = await ctx.db.insert('relays', {
      ...
      origin: { kind: 'panel-node', backendServerId: serverId, nodeName: 'node-a' },
      nodeName: 'node-a',
    ...
    const listenerId = await ctx.db.insert('relayListeners', {
      ...
      transport: 'tcp',
      originPort: 443,
      tlsNames: [{ name: 'www.example', status: 'active' }],
      realityTarget: { address: 'target.example', port: 443 },
      matchRule: { kind: 'remark', remark: 'node-a-relay-a1' },
      panelBinding: { inboundTag: 'T', configProfileUuid: 'cp', configProfileInboundUuid: 'in' },
      source: 'role',
      configHash: 'h',
      enabled: true,
      deployed: true,
