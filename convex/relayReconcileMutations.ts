...
import { checkPublishable } from './relayOrigins';
...
export const markDestroyed = internalMutation({
  ...
  handler: async (ctx, { edgeId }) => {
    ...
    const origin = await ctx.db.get(edge.originId);
    if (origin) {
    ...
    await writeAuditLog(ctx, {
      ...
      targetId: edgeId,
      payload: { originSlug: origin?.slug ?? '', provider: edge.provider ?? null, edgeId },
    });
    return null;
  },
});
...
export const publishStandby = internalMutation({
  args: { originId: v.id('relayOrigins'), candidates: v.array(v.id('relayEdges')) },
  handler: async (
    ctx,
    { originId, candidates },
  ): Promise<{ published: boolean; rotationId: Id<'relayRotations'> | null }> => {
    ...
    for (const edgeId of candidates) {
      const edge = await ctx.db.get(edgeId);
      if (!edge || edge.originId !== originId) continue;
      ...
      if (idx === 0 && origin.hostManaged) {
        ...
        const now = Date.now();
        const rotationId = await ctx.db.insert('relayRotations', {
          originId,
          kind: 'publish',
          trigger: 'reconcile',
          burn: false,
        ...
        await ctx.db.patch(originId, { activeRotationId: rotationId, updatedAt: now });
        await ctx.scheduler.runAfter(0, internal.relayRotations.step, { rotationId });
        await ctx.db.patch(rotationId, { nextStepAt: now });
        await writeAuditLog(ctx, {
          actorType: 'system',
          action: 'admin.relay.origin.provision',
