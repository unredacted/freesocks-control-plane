 ...
 * gone, so the route would otherwise fetch → backend 404 → answer a misleading
...
export const insertSubscription = internalMutation({
  args: {
    ...
    // the chosen mode group) — read back by lifecycle.activeSubAndTier so tier pushes
  ...
  handler: async (ctx, a) => {
    ...
    const inserted = await ctx.db.insert('subscriptions', {
      ...
      subToken,
      // Origin-edge assignment key: opaque, per key, never the subToken (so a
      // fronted-URL rotation does not reshuffle a member's endpoints).
      renderKey: randomHex(32),
      state: 'active',
      updatedAt: Date.now(),
    });
    return inserted;
...
/**
 * Lazily mint the origin-edge assignment key for rows that predate it (every new
...
export const ensureRenderKey = internalMutation({
  ...
  handler: async (ctx, { subscriptionId }): Promise<string | null> => {
    ...
    if (!sub) return null;
    if (sub.renderKey) return sub.renderKey;
    const renderKey = randomHex(32);
    await ctx.db.patch(subscriptionId, { renderKey });
    return renderKey;
  },
});

// Delivery stamps are throttled so a polling client doesn't turn every cache hit
// into a row write; `contentAt` (when the served body was generated) always
// advances when newer content is delivered.
const DELIVERY_STAMP_THROTTLE_MS = 60_000;

/**
 * Record a successful fronted delivery (GET /api/v1/sub/<token> returned 200):
 * `lastDeliveredAt` = now, `lastDeliveredContentAt` = when the served content
 * was generated by the backend (a cache hit serves older content than "now"),
 * and — when the body was origin-rendered — `lastRenderedEpoch` = the origin's
 * publication epoch it was rendered against (the cache entry's token). The
 * origin layer compares that epoch with the origin's current one to tell
...
export const markDelivered = internalMutation({
  args: {
    ...
    /** Present when this delivery was an origin render: persisted as `lastRender`. */
...
/**
 * Record that a subscription's S3 mirror now holds content generated at
 * `contentAt` (rendered against `renderedEpoch` when origin rendering applied).
 * A mirror-only member never hits the fronted route, so without this stamp the
 * origin heuristics (refresh nudge, report attribution) would treat them as
 * never having seen post-rotation content. `lastDeliveredAt` (a fronted 200)
 * is deliberately NOT touched.
 */
export const markMirrorRefreshed = internalMutation({
  args: {
    subscriptionId: v.id('subscriptions'),
    contentAt: v.number(),
    renderedEpoch: v.optional(v.union(v.number(), v.null())),
  ...
  handler: async (ctx, { subscriptionId, contentAt, renderedEpoch }) => {
    ...
    if ((sub.lastDeliveredContentAt ?? 0) < contentAt) patch.lastDeliveredContentAt = contentAt;
    if (typeof renderedEpoch === 'number' && renderedEpoch !== sub.lastRenderedEpoch)
      patch.lastRenderedEpoch = renderedEpoch;
    if (Object.keys(patch).length > 0) await ctx.db.patch(subscriptionId, patch);
    return null;
  },
});
 ...
 * serving the cached node's links. Cheap — no backend call, no new key, and the
...
export interface MirrorContext {
  ...
  backendServerId: Id<'backendServers'> | null;
  backendShortId: string;
  /** The backend-provided public subscription URL — where the raw content actually lives. */
  subscriptionUrl: string;
 ...
 * switch that PATCHes the Remnawave user's mode group rather than re-issuing). Updates
 ...
 *  after a backend re-registration, or a legacy row never had one). Every
