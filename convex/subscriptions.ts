...
export const insertSubscription = internalMutation({
  ...
  handler: async (ctx, a) => {
    ...
    const inserted = await ctx.db.insert('subscriptions', {
      ...
      subToken,
      // Relay-edge assignment key: opaque, per key, never the subToken (so a
 ...
 * was generated by the panel (a cache hit serves older content than "now"),
 * and — when the body was relay-rendered — `lastRenderedEpoch` = the origin's
 * publication epoch it was rendered against (the cache entry's token). The
 * relay layer compares that epoch with the origin's current one to tell
 ...
 * `contentAt` (rendered against `renderedEpoch` when relay rendering applied).
 * A mirror-only member never hits the fronted route, so without this stamp the
 * relay heuristics (refresh nudge, report attribution) would treat them as
