 ...
 * The relay layer uses this as "has this key seen post-rotation content".
 */
export const markDelivered = internalMutation({
  args: { subscriptionId: v.id('subscriptions'), contentAt: v.number() },
  handler: async (ctx, { subscriptionId, contentAt }) => {
    const sub = await ctx.db.get(subscriptionId);
    if (!sub) return null;
    const now = Date.now();
    ...
    if (!contentAdvanced && !stale) return null;
