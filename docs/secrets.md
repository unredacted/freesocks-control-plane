...
| `ACCOUNT_ID_PEPPER`         | Peppered keyed hash of the 32-digit account number. Also the root the backend-digest key is derived from (`docs/servers.md`), which is a second reason it must never change | **SET ONCE.** Rotating **invalidates every account number** — every member is locked out permanently. Never rotate on a live deployment.                                                |
...
| `NOWPAYMENTS_*`, `BTCPAY_*`, `STRIPE_*`, `PAYPAL_*`, `PUBLIC_BASE_URL`  | Per billing rail (see `docs/billing.md`). Required once a rail is enabled in Admin → Billing. The `BTCPAY_*` values come from your own BTCPay Server (an API key scoped to **both** `btcpay.store.cancreateinvoice` and `btcpay.store.canviewinvoices` — the read-back is required for the settle cross-check, see `docs/billing.md` — plus the store-webhook secret you choose); rotating them is a re-issue on that server. |
