...
- [`docs/billing.md`](billing.md): self-service membership purchases — self-upgrade + **gift codes** — and the NOWPayments USD off-ramp ops.
- [`docs/btcpay-server-runbook.md`](btcpay-server-runbook.md): provisioning and operating the BTCPay Server itself — host, minimum version (2.4.2), `pay.freesocks.org` hostname gotchas, the three-key inventory, chains, backups, upgrades.
...
- **Self-service membership billing** (`convex/billing.ts`, `convex/lib/processors/*`,
  ...
  scaffolded behind admin toggles) and DORMANT until switched on** — `billingConfig` ships every rail
  `enabled:false` with placeholder prices, so no billing is active until an admin sets keys/prices and
  enables a rail. "Live" elsewhere in this file means _the code path is production-ready_, not that the
  rail is currently taking money; per-rail state is whatever Admin → Billing says. **BTCPay additionally
  needs a server that does not yet exist** — provisioning is `docs/btcpay-server-runbook.md`. The USD
  off-ramp is an ops runbook, and it covers only the NOWPayments leg (NOWPayments → USDC →
  Coinbase/Kraken → ACH); crypto received into the org's own BTCPay wallet has **no documented or
  automated path to USD** (BTCPay removed its exchange/custodian integration in 2.0.0).
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ------------------------------------------------------ |
| **Self-service billing**: the crypto rails (NOWPayments, **BTCPay** — self-hosted Bitcoin/Lightning) are **Live**; **Stripe + PayPal** are implemented, wired, unit-tested, and **sandbox-dry-run-verified** (2026-07 operator pass) — enabling them is now just the admin toggle. Remaining is the non-code launch checklist: NOWPayments US-nonprofit ToS confirmation + opening the USDC→Coinbase/Kraken→ACH off-ramp account (the runbook itself is documented in `docs/billing.md`). Admin → Billing flags enabled-but-misconfigured rails (readiness) and has a **live per-processor credential probe** (`POST /api/v1/admin/billing/test-connection` — shipped 2026-07-18). | M    | `docs/billing.md`, this file (§1.6/§1.7)               |
| **Native-speaker translation review** + extracting remaining marketing copy into i18n keys (the non-English locales are a first-pass MT; the critical journey strings are done). All 4 locales are at 0/768 missing keys (2026-07-18); what remains is a native-quality pass over the MT text itself (review packets in `translation-review/`).                                                                                                                                                                                                                                                                                                                                    | M    | this file (§1.7)                                       |
...
| **Remnawave 3.x port**: the provider speaks the 2.x contract only — Remnawave 3.0 dropped the user `uuid` (numeric `id` + `shortUuid`; every `/api/users/{uuid}` route changed), removed the realtime node-bandwidth endpoint, and 2.8.1 made `APP_SECRET` mandatory. Verified 2026-09-01 against 3.4.2 with the live integration harness. Until ported, the real panels stay on 2.8.x, the test panel is pinned to 2.8.0, and Dependabot ignores `remnawave/backend` majors.                                                                                                                                                                                                      | L    | `docs/backends.md`, `convex/lib/backends/remnawave.ts` |
