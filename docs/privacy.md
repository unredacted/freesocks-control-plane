...
## 8. Relay edges: reachability probes and report attribution

The relay-edge layer (`docs/relays.md`) adds two flows that touch third parties or
...
- **Reachability probes** ask measurement services (Globalping, check-host.net,
  ...
  edge. Probes are off by default (`relay.probe.enabled`); tokens for the
...
- **Report attribution** labels a member's issue report with the relay origin
  ...
  window is enforced through a peppered HMAC mark (`RELAY_MARK_PEPPER`, falling
