...
| `RELAY_MARK_PEPPER`                              | optional: HMAC pepper for the relay block detector's per-member-per-window dedupe mark (`docs/relays.md`); falls back to `IP_HASH_SALT`. `openssl rand -hex 32`                                                                 |
| `RELAY_PROBE_GLOBALPING_TOKEN`                   | optional env fallback for the Globalping probe token (normally a write-only setting on Admin → Relay edges)                                                                                                                     |
| `RELAY_PROBE_RIPEATLAS_KEY`                      | optional env fallback for the RIPE Atlas probe key (same)                                                                                                                                                                       |
...
> that. It never regenerates an already-set value. The deployer prints the
> generated `ADMIN_BOOTSTRAP_SECRET` (for the first passkey) in its log on every
> deploy; it's also visible on the dashboard's env screen. (A bare
> `bunx convex env get` from the host shell fails — see the note below.) A
> standalone `bunx convex deploy` (no compose) still needs them set manually.
...
| `WEBAUTHN_RP_NAME`                                                             | `FreeSocks Admin`                    | admin passkey display name                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `WEBAUTHN_RP_NAME_MEMBER`                                                      | `FreeSocks`                          | member (opt-in) passkey display name — members and admins register against the same RP ID but see different names                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `TRUSTED_PROXY`                                                                | unset                                | legacy alias for `TRUSTED_PROXY_HOPS=1` — set `true` when Caddy is the public edge (topology (a)) and overwrites `X-Forwarded-For`. **Required behind a single Caddy** — unset (and no `TRUSTED_PROXY_HOPS`) means anonymous-issuance fails closed with `freetier.ip_unresolved`.                                                                                                                                                                                                                         |
| `TRUSTED_PROXY_HOPS`                                                           | unset (`TRUSTED_PROXY=true` ≡ 1)     | integer ≥1: trusted appending reverse-proxy hops counted from the RIGHT of `X-Forwarded-For`. `1` = Caddy is the edge; `2` = one proxy fronts Caddy (Pangolin / CF Tunnel / ngrok / LB — also set the compose var `CADDY_TRUSTED_PROXIES` on the `web` service to that peer's IP/CIDR); +1 per extra hop. Wins over `TRUSTED_PROXY`; garbage/<1 → fail closed. Verify via `GET /api/v1/admin/client-ip`. See `docs/beta-deploy.md` §"Client IP".                                                          |
...
| `POP_REQUIRED`                                                                 | unset                                | CDN-blinding Phase 2: set `true` (after the client soaks) to reject legacy cookie-only sessions and require proof-of-possession. Sessions already bound to a PoP key always enforce it regardless. Watch the admin dashboard's _Session protection_ card (`statusSummary.pop.readyToEnable`) for readiness before flipping. Already flipped on beta (2026-07). See `docs/threat-model-cdn-blinding.md`.                                                                                                   |
...
| `SUBSCRIPTION_RETENTION_DAYS`                                                  | `90`                                 | retention for long-`deleted` subscription rows (history only; the backend user is already gone). Other `*_RETENTION_DAYS` knobs: `AUDIT` (180), `WEBHOOK` (90), `TIER_HISTORY` (365).                                                                                                                                                                                                                                                                                                                     |
...
| `BTCPAY_API_URL`, `BTCPAY_STORE_ID`, `BTCPAY_API_KEY`, `BTCPAY_WEBHOOK_SECRET` | none                                 | self-hosted Bitcoin rail (on-chain + Lightning) via the operator's own BTCPay Server — so there is no default API URL. Required once the BTCPay rail is enabled; while the webhook secret is unset `/api/webhooks/btcpay` answers `503 billing.not_configured`. The API key needs **both** `btcpay.store.cancreateinvoice` and `btcpay.store.canviewinvoices` (the settle cross-check reads the invoice back). Minimum server version **2.4.2**. See `docs/billing.md` + `docs/btcpay-server-runbook.md`. |
...
```caddy
{
    # Client-IP trust: leave the sentinel default when Caddy is the public edge.
    # If a proxy fronts Caddy (Pangolin / CF Tunnel / ngrok / LB), set
    # CADDY_TRUSTED_PROXIES to its peer IP/CIDR + TRUSTED_PROXY_HOPS=2 on the
    # backend. See docs/beta-deploy.md §"Client IP".
    servers {
