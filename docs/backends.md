...
| update          | `PATCH /api/users`                                                                                                                    | **`uuid` is in the request BODY, not the path** (the route has no path param; the DTO requires `uuid` or `username`). The DTO takes `.optional()` NOT `.nullable()` for `trafficLimitBytes`/`expireAt`, and refuses past expiry dates — the provider coerces `null` traffic → `0` (the panel's unlimited sentinel), omits a `null` expireAt, and clamps a past expireAt to now+5min (FCP's grace sweep governs actual disablement) |
