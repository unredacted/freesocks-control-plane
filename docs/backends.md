...
verbatim. Nothing outside the dispatch converts, and the admin UI shows the stored form. The provider tests assert the paths, so a drift here fails CI — this is the guard
...
3. **Re-key that panel's subscriptions** from the deployer container (bare `bunx convex run`
   ...
   ```sh
   bunx convex run backendServers:migrateRemnawaveUserIds '{"dryRun": true}'
...
implies `hostManagement` (pinned by `capabilities.test.ts`). The pure mapper from
`PanelInbound[]` to listener candidates lives in `convex/lib/edges/inboundMapping.ts`
(`docs/edges.md` § "Listener catalogue", "Discovery"). Neither op has a dev mock branch: the
Host ops have none either (the fake edge provider stops at the provider layer).
