 ...
 * (the dark default). The whole hpke module - and its heavy @hpke/@noble crypto
...
let _hpke: Promise<typeof import('./hpke')> | null = null;
const loadHpke = () => (_hpke ??= import('./hpke'));
...
async function request<S extends z.ZodTypeAny>(
  ...
  const seal = HPKE_ENABLED
    ? await (await loadHpke()).prepareOutbound(path, method, outBodyStr)
    : undefined;
  ...
  if (seal) json = await (await loadHpke()).openInbound(seal, path, method, json);
